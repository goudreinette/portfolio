<?php function thb_autotype( $atts, $content = null ) {
  ...
  extract( $atts );
	...
	$out = $text = '';
	$element_id = 'thb-autotype-' . mt_rand(10, 99);
	...
	ob_start();
	?>
	<div id="<?php echo esc_attr($element_id); ?>" class="thb-autotype">
		<?php 
			...
			echo str_replace('<placeholder>', '<div class="thb-autotype-entry" data-thb-loop="'.esc_attr($loop).'" data-strings="'.esc_attr(json_encode($autotype)).'" data-speed="'.esc_attr($typed_speed).'"></div>', $typed_text_safe);
		?>
	</div>

...
?>
<!-- Start Header -->
<header class="header style2 <?php echo esc_attr($menu_style); ?>">
			...
			<div class="small-12 columns portfolio-header">
				<div class="thb-breadcrumb-holder">
					...
					<?php if (is_singular('portfolio')) { ?>
						<a href="<?php echo esc_url(home_url()); ?>" title="<?php bloginfo('name'); ?>"><?php esc_html_e('Projects', 'werkstatt'); ?></a>
					<?php } else { ?>
						<a href="<?php echo esc_url(get_permalink( get_option( 'page_for_posts' ) )); ?>" title="<?php bloginfo('name'); ?>"><?php esc_html_e('Blog', 'werkstatt'); ?></a>
					<?php } ?>
					<span><em>-</em> <?php echo get_the_title(get_queried_object_id()); ?></span>
				</div>

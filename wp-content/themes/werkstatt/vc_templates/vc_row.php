...
/* Row Title */
if ( $thb_row_title ) {
	$wrapper_attributes[] = 'data-row-title="' . esc_attr( $thb_row_title ) . '"';
}
/* Midnight */
//if ( $thb_color ) {
...
if ( $thb_video_bg !== '' ) {
	...
	if ( !empty($parallax_image_src) ) {
		...
		$attr_strings[] = 'poster="' . esc_url($parallax_image_src) . '"';
	}
	
	
	$bg_video .= sprintf( '<video %s class="thb-row-video-bg">', join( ' ', $attr_strings ) );

	$video_source_html = '<source type="%s" src="%s" />';

	$video_type = wp_check_filetype( $thb_video_bg, wp_get_mime_types() );
	$bg_video .= sprintf( $video_source_html, $video_type['type'], esc_url( $thb_video_bg ) );


	$bg_video .= '</video>';
	
	if ( $thb_video_overlay_color != '' ) {
		$bg_video .= '<div class="thb_video_overlay" style="background-color: '.esc_attr($thb_video_overlay_color) .';"></div>';
	}
...
$output .= '<div class="scroll-bottom '.$thb_scroll_bottom_color.'"><div></div></div>';	
}
$output .= '</div>';
$output .= $after_output;

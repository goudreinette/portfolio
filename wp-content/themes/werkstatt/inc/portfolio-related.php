...
/* Header Filter */
function thb_portfolio_categories($categories, $id, $style) {
	...
	<nav class="thb-portfolio-filter <?php echo esc_attr($style); ?>" id="thb-filter-<?php echo esc_attr($id); ?>">
		...
		<?php } ?>
			<ul>
				<li><a href="#" data-filter="*" class="active"><?php esc_html_e('All', 'werkstatt'); ?></a></li>
			<?php 
				 foreach ($categories as $cat) {
				 	$term = get_term($cat);
				 	echo '<li><a href="#" data-filter=".thb-cat-' . $term->slug . '">' . $term->name . '</a></li>';
				 }
			?>
			</ul>
		</strong>
...
add_action( 'thb-render-filter', 'thb_portfolio_categories', 1, 3 );
...
function thb_show_all($portfolios) {
	...
	<nav class="thb-show-all">
		...
		<div class="items custom_scroll" id="thb-show-all" data-horizontal="1">
...
/* Portfolio Preloader */
function thb_portfolio_preloader() {
	$preloader = ot_get_option('thb_preloader', 'hexagon');
	if ($preloader !== 'no') {
		if ($preloader == 'hexagon') {
			?>
			<div class="thb-preloader"><?php get_template_part( 'assets/img/preloader-hexagon.svg' ); ?></div>
			<?php
		} else if ($preloader == 'circle') {
			?>
			<div class="thb-preloader"><?php get_template_part( 'assets/img/preloader-circle.svg' ); ?></div>
			<?php
		}
	}
}
		...
		<div class="portfolio-attributes <?php if(isset($style)) { echo esc_attr($style); } ?>">
			...
			<?php if (!empty($client_more)) { ?>
				<?php foreach ($client_more as $more ) { ?>
					<div class="attribute"><strong><?php echo esc_attr($more['title']); ?></strong> <?php echo $more['client_custom_value']; ?></div>
				<?php } ?>
			<?php } ?>
		</div>
	<?php
	...
	<div class="thb-portfolio-share">
		<strong>
			...
			<ul>
				...
				<li><a href="<?php echo 'https://twitter.com/intent/tweet?text=' . htmlspecialchars(urlencode(html_entity_decode($title, ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8') . '&url=' . urlencode( esc_url( $permalink ) ) . '&via=' . urlencode( get_bloginfo( 'name' ) ) . ''; ?>" class="social twitter"><i class="fa fa-twitter"></i></a></li>
				<?php } ?>
...
function thb_portfolio_video($portfolio_header_video) {
	global $wp_embed; 
	$src = wp_get_attachment_url($portfolio_header_video);
	$meta = wp_get_attachment_metadata($portfolio_header_video);
	$portfolio_header_video_loop = get_post_meta(get_the_ID(), 'portfolio_header_video_loop', true) !== 'off' ? 'on' : false;
	echo wp_video_shortcode(array(
		'src' => $src,
		'width' => $meta['width'],
		'height' => $meta['height'],
		'autoplay' =>'on',
		'loop' => $portfolio_header_video_loop,
		'preload' => 'auto'
	));
}
add_action( 'thb_portfolio_video', 'thb_portfolio_video', 3, 3 );
  ...
  $el_class[] = $thb_style === 'style1' ? $thb_full_height : '';
	...
	<div class="thb-device <?php echo esc_attr($thb_align); ?>">
		<div class="thb-iphone">
			...
			<div class="screen">
	<?php } ?>
				<div id="<?php echo esc_attr($element_id); ?>" class="<?php echo implode(' ', $el_class); ?>" data-pagination="<?php echo esc_attr($thb_pagination); ?>" data-navigation="<?php echo esc_attr($thb_navigation); ?>" data-center="0" data-columns="1" data-infinite="false">
					<?php
						foreach ($images as $image) {
							...
							?>
								<figure style="background-image: url(<?php echo esc_attr($image_link[0]); ?>);">
									<?php if($lightbox) { ?>
										<a href="<?php echo esc_attr($image_link[0]); ?>" data-size="<?php echo esc_attr($image_link[1].'x'.$image_link[2]); ?>"></a>
									<?php } ?>
								</figure>
							<?php
							} else {

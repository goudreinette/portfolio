<?php function thb_instagram( $atts, $content = null ) {
  ...
  extract( $atts );
	...
	//if (false === ($instagram = get_transient('thb-instagram-media-'.$access_token)) && $username && $access_token) {
		...
		set_transient('thb-instagram-media-'.$access_token, $instagram, HOUR_IN_SECONDS);
	//}
	...
	?>
		<div class="row <?php echo esc_attr($class); ?> instagram-row">
			<?php
				if (!$access_token) {
					esc_html_e('Please fill out Instagram Settings inside Appearance > Theme Options', 'werkstatt');
				} else if ($instagram) {
					foreach ($instagram as $item) {
						echo '<div class="small-6 '.$columns.' columns"><figure style="background-image:url('. esc_url($item['image_url']) .')">';
						if ($link == 'true') {
